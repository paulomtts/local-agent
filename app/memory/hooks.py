...
from app.memory.episodic import extract_episodic_memory
from app.memory.queries import WORKING_FILE
from app.memory.semantic import extract_semantic_memory
...
def _write_working_memory(items: list[ContextItem[MemoryItem]], added: int) -> None:
    content = "\n".join(str(item.content) for item in items)
